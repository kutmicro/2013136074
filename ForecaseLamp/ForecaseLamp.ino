...
int LEDtype = 0;    // 0 : 디폴트(RainbowCycle), 1 : Rainbow, 2: 기온에따라, 3 : 사용자 지정 RGB
String RGB = "";
String TEMP = "";
...
void controlLED(String weather, int temp) {
  if (weather.equals("Snow")) {  //눈오면 1번 LED켜기
    ...
    switch(LEDtype) {
      ...
      break;
      case 2:
      colorWipe(strip1.Color(255, 0, 0), setWait, &strip1);
      break;
      case 3:
      //첫번째 RGB
    ...
    switch(LEDtype) {
      ...
      break;
      case 2:
      colorWipe(strip2.Color(255, 0, 0), setWait, &strip2);
      break;
      case 3:
      //두번째 RGB
    ...
    switch(LEDtype) {
      ...
      break;
      case 2:
      colorWipe(strip3.Color(255, 0, 0), setWait, &strip3);
      break;
      case 3:
      //세번째 RGB
      String tmpStr = RGB.substring(RGB.indexOf("(") + 1);
    ...
    switch(LEDtype) {
      ...
      break;
      case 2:
      colorWipe(strip4.Color(255, 0, 0), setWait, &strip4);
      break;
      case 3:
      //네번째 RGB
      String tmpStr = RGB.substring(RGB.indexOf("(") + 1);
...
void LEDParsing(String data) {
  ...
  else if(data.startsWith("TEMP")) {    //RGB 다섯개 데이터
    TEMP = data.substring(data.indexOf("(") + 1, data.lastIndexOf(")"));    //"TEMP"와 필요없는 괄호 제거
    LEDtype = 2;
  }
